...
class DQNBaseAgent(ABC):
    ...
    def record_evaluate(self, load_path, store_path="./record.mp4", seed=42):
        import gym
        import random
        import numpy as np
        random.seed(seed)
        ...
        self.load(load_path)
        video_recorder = gym.wrappers.monitoring.video_recorder.VideoRecorder(self.test_env, path=store_path)
        
        observation, info = self.test_env.reset(seed=seed) # Choose one episode.
        total_reward = 0
        while True:
            self.test_env.render()
            action = self.decide_agent_actions(observation, self.eval_epsilon, self.test_env.action_space)
            ...
            if terminate or truncate:
                print(f"total reward: {total_reward}w")
                break
    ...
    def args_parse(self):
        parser = argparse.ArgumentParser(description="Evaluate a model and record the evaluation.")
        parser.add_argument("--load_path", default="", type=str, help="Path to the trained model.")
        parser.add_argument("--store_path", type=str, default="./record.mp4", help="Path to save the recorded video.")
        parser.add_argument("--mode", type=str, choices=["record_eval", "train"], default="train", help="Mode of evaluation or other modes.")
