...
/* 
   Big:    1372, 2568, 1257, 2134 
   Little: 4312, 7521, 8652, 2731
*/
class board {
public:
...
public:
	...
	virtual float estimate(const board& b) const {
		// TODO
		/* 對於每個 Pattern 來說, 每個 isomorphism 都要進行計算得到權重和 */
		float value = 0;		
		for (size_t i = 0; i < isomorphic.size(); i++)
		{
	...
	virtual float update(const board& b, float u) {
		// TODO
		/* 對於每個 Pattern 來說, 要計算 Pattern 對於每個 isomorphism 的 error 值並調整 Pattern 的權重 
		 * u: error value
		 */
		float adjust = u / isomorphic.size();
		float value = 0;
		for (size_t i = 0; i < isomorphic.size(); i++)
		{	
			size_t index = indexof(isomorphic[i], b);
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
		// TODO
		/* 取出每個 Pattern 的值，在這次實驗中最高只有看到 16384-tile, 所以是 14 種，仍然是可以被表示的範圍。
		 * 當指定 Pattern 所取出的 6 個 tiles 的值為 1024,4,8,16,32,64 時，所得到的 index 為 65432a。
		 */
        size_t index = 0;
		for (size_t i = 0; i < patt.size(); i++)
		{	
            size_t val = b.at(patt[i]); 
			index |= val << (i << 2); 
		}
		return index;
	}
...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
                /*  Steps:
                    1. 考慮 board 的 after_state, 即移動後但尚未出現 2, 4 的時候 
                    2. 先蒐集每個非零 tile 的 index
                    3. 對每個非零 tile 產生 2 或 4 的期望盤面分數
                    4. 對每個動作產生的 Reward 和期望盤面分數相加，並取移動後最高的盤面分數作為 best state 回傳。
                 */
				float value = 0;
				...
				for (int pos : empty_tiles)
				{
					board temp = after_state;
					temp.set(pos, 1);
					value += 0.9 * estimate(temp);

					temp = after_state;
					temp.set(pos, 2);
					value += 0.1 * estimate(temp);
				}

                value /= (empty_tiles.size() + 1e-9);

				move->set_value(value + move->reward());
				...
				/* due to numeric error, in afterstate version we should set value to a small real number */
				// move->set_value(1e-24);
			}
			debug << "test " << *move;
		}
		return *best;
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		// TODO
		/* TD(0) 的公式為 V(S_t) <- V(S_t) + a( R_{t+1} + V(S_{t+1}) - V(S_t) )   
		 * 意思是 比較當前狀態 S_t 的估計值和從下一狀態 S_{t+1}(這裡是 afterstate: path[t].value()) 得到的回報之間的差異，然後根據這個差異來調整當前狀態的估計值
		 * 其中調整差異的部分在 update() 中被完成。
		 */ 
		
		// State version is easier.
    	for (int t = 0; t < path.size(); ++t) {
			float next_value = (t+1 < path.size()) ? (path[t].reward() + estimate(path[t+1].before_state())) : path[t].reward();
			float error = next_value - estimate(path[t].before_state());
    	    update(path[t].before_state(), alpha * error);	
    	}

		// after_state learning
    	// for (int t = 0; t < path.size(); ++t) {
		// 	float next_value = (t+1 < path.size()) ? (path[t].reward() + path[t+1].value()) : path[t].reward();
		// 	float error = next_value - path[t].value();

		// 	board after_state = path[t].after_state();
		// 	std::vector<int> empty_tiles;
		// 	for (int i = 0; i < 16; i++)
		// 	{
		// 		if (after_state.at(i) == 0)
		...
		// 		board temp = after_state;
		// 		temp.set(pos, 1);
		// 		update(temp, 0.9 * error * alpha);

		// 		temp = after_state;
		// 		temp.set(pos, 2);
		// 		update(temp, 0.1 * error * alpha);
		// 	}
    	// }
	}
...
int main(int argc, const char* argv[]) {
	...
	tdl.load("./checkpoints/weight_beforestate.pth");
