...
class CarRacingEnvironment:
	...
	def step(self, action):
		...
		# 路的占比越大, 成績應該越高 (希望保持道路中央)
		ratio = (road_pixel_count / (road_pixel_count + grass_pixel_count))
		
		addtion_reward = - (0.01 * grass_pixel_count)
		# addtion_reward = 0
		# if ratio >= 0.8:
		# 	addtion_reward += 0.05
		# else:
		...
		reward += addtion_reward
  
		# 原本的 reward 算法可能已經會促使車前進了, 不需要加上 progress reward
		# progress_reward = '???'
  
  		# 看超出範圍時有多超, 草越大代表要給予更多的懲罰
	...
	# demo 3128, 6727, 8843, 7021, 2712
	def reset(self, seed):
		obs, info = self.env.reset(seed=seed) #fill seed here for evaluation
		self.ep_len = 0
		obs = cv2.cvtColor(obs, cv2.COLOR_BGR2GRAY) # 96x96
...
if __name__ == '__main__':
	env = CarRacingEnvironment(test=False)
	obs, info = env.reset()
	done = False
	total_reward = 0
	total_length = 0
