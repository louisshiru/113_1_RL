...
class GaussianNoise:
	...
	def generate(self):
		return np.random.normal(self.mu, self.std).astype('float32')
...
class TD3BaseAgent(ABC):
	...
	def record_evaluate(self, load_path, seed=42):
		import gym
		import random
		import numpy as np
		...
		# """ uncomment this for record video
		...
		for seed in seeds:
			video_recorder = gym.wrappers.monitoring.video_recorder.VideoRecorder(self.test_env.env, path=f"{seed}.mp4")
			
			state, info = self.test_env.reset(seed=seed) # Choose one episode.
			total_reward = 0
			while True:
				self.test_env.env.render()
				action = self.decide_agent_actions(state)
				next_state, reward, terminates, truncates, _ = self.test_env.step(action)
				total_reward += reward
		...
		for i in range(0, 5):
			print(f"seed={seed}, total reward: {all_rewards[i]}")

		avg = sum(all_rewards) / len(seeds)
		print(f"average score: {avg}")
		# """
