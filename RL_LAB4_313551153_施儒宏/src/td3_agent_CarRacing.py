...
class CarRacingTD3Agent(TD3BaseAgent):
    ...
    def update_behavior_network(self):
        ...
        with torch.no_grad():
            ...
            # 為什麼 DDPG 需要 continueous space ?
            # 在離散空間中沒辦法加上雜訊，而沒辦法做平滑化
            # DDPG 這裡需要使用 Contiguous 使得 policy network 可以直接輸出動作來優化網路
            # 而離散空間沒辦法定義 1.1, 1.3, 1.6 這些動作到底是什麼東西。
            # 實務上來說沒辦法使用定值來規範每個人在空間上的動作 (角度, 速度, 推力)
            # 連續方法能夠直接優化目標, 而不需要將動作空間離散化, 避免引入額外的離散化誤差
            # 使用 DDPG 能更平滑的更新連續動作, 而不是在離散空間動作中直接跳躍
            # 連續方法在高維動作空間中表現得更好
